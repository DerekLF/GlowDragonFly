...
void loop() {
  ...
  switch (Modes) {
    ...
    case Travel2:  //Light travels from the center of the dragonfly to the outside
      returnVar = mode_Travel_2(0, 0, arDATALoop);
      break;
...
int mode_Travel_1(int intervalT1, int arDATA[4]) {  //travel from tail tip to head
  ...
  if (currentMillisT1 - previousMillisT1 >= intervalT1) {
    ...
    switch (state) {
      ...
      case 1:  // Tail; end to tip
        ...
        for (int i = PulseWidth; i > 0; i--) {
          tail.setPixelColor(travelLength - (position - (PulseWidth / 2) + i), tail.Color(arRGB[0], arRGB[1], arRGB[2], arRGB[3]));
        }
        position++;
      ...
      case 2:  // Back wings; begin to end, both sides
        ...
        for (int i = 0; i < PulseWidth; i++) {
          wingLB.setPixelColor(position - (PulseWidth / 2) + i, wingLB.Color(arRGB[0], arRGB[1], arRGB[2], arRGB[3]));
          wingRB.setPixelColor(position - (PulseWidth / 2) + i, wingRB.Color(arRGB[0], arRGB[1], arRGB[2], arRGB[3]));
        }
        position++;
        break;
      ...
      case 3:  // Front wings; begin to end, both sides
        travelLength = Num_Leds_Wings;
        for (int i = 0; i < PulseWidth; i++) {
          wingLF.setPixelColor(position - (PulseWidth / 2) + i, wingLF.Color(arRGB[0], arRGB[1], arRGB[2], arRGB[3]));
          wingRF.setPixelColor(position - (PulseWidth / 2) + i, wingRF.Color(arRGB[0], arRGB[1], arRGB[2], arRGB[3]));
        }
        position++;
        break;
    ...
    if (position >= travelLength + PulseWidth || state == 0) {
...
int mode_Travel_2(int wait, int swoosh, int arDATA[4]) {  //travel from body to ends
}
