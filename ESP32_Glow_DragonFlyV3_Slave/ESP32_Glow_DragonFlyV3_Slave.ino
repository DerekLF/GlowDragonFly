...
void loop() {
...
#endif
      returnVar = mode_Heartbeat(espData.speed);
      break;
    case Wing:  //Light slowly travels across it's wings matching it's flapping speed
#if DEBUG > 1
      Serial.println("Entering mode: Wing");
#endif
      returnVar = mode_Wing(espData.speed, arDATALoop);
      break;
    case Blink:  //Similar to heartbeat, just a different color
#if DEBUG > 1
      Serial.println("Entering mode: Blink");
#endif
      //returnVar = mode_Wing(espData.speed, arDATALoop);
      break;
  }
  ...
  if (returnVar == 1) {
    espData.animationMode = espDatabuf.animationMode;
    espData.speed = espDatabuf.speed;
    espData.colorCombo = espDatabuf.colorCombo;
    espData.interval = espDatabuf.interval;
    espData.checksum = espDatabuf.checksum;
  }
}
...
void OnDataRecv(const uint8_t* mac, const uint8_t* incomingData, int len) {
  if (len == sizeof(usefulInfo)) {
    ...
    memcpy(&espDatabuf, incomingData, sizeof(espDatabuf));
#if DEBUG > 0
    // Print received data to Serial
    Serial.println("Data received:");
    Serial.print("Animation Mode: ");
    Serial.println(espDatabuf.animationMode);

    Serial.print("Speed: ");
    Serial.println(espDatabuf.speed);

    Serial.print("Color Combo: ");
    Serial.println(espDatabuf.colorCombo);

    Serial.print("Interval: ");
    Serial.println(espDatabuf.interval);

    Serial.print("Checksum: ");
    Serial.println((uint64_t)espDatabuf.checksum, HEX);
    Serial.println();
#endif
  } else {
